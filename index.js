...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    for (let anju of vien){
				if (budy === anju){
					...
					Tony.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			//auto sticker
     ...
     break
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await Tony.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						Tony.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *TONY 𝐁𝐨𝐭 BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					Tony.sendMessage(_.jid, `*TONY 𝐁𝐨𝐭 BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
